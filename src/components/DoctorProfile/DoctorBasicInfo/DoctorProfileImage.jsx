...
import {doctorProfileActions, headerActions} from '../../../_actions';

/**
 * DoctorProfileImage
 *
 * @package                SafeHealth
 * @subpackage             DoctorProfileImage
 * @category               Component
 * @DateOfCreation         22 May 2018
 * @ShortDescription       This component is reponsible for update basic profile
 */
class DoctorProfileImage extends React.Component {
  ...
  handleSave(){
    const { dispatch }  = this.props;
    dispatch(doctorProfileActions.updateProfileImage(this.state.croppedImage));
  }
...
function mapStateToDispatch(state) {
    const {isUserNotValid, successMessage,errorMsg} = state.doctorProfile;
    return {
        isUserNotValid,
        successMessage,
        errorMsg
...
const connectedDoctorProfileImage = connect(mapStateToDispatch)(DoctorProfileImage);
export { connectedDoctorProfileImage as DoctorProfileImage };
