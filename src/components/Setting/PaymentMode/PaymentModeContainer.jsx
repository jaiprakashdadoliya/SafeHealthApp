...
import formConfig from './PaymentModeConfig';
import moment from 'moment';
import {paymentModeActions} from './paymentModeActions';

const PaymentMode = Loadable({
    loader: () => import('./PaymentMode' /* webpackChunkName = "PaymentMode" */).then(object => object.PaymentMode),
    loading: Loading
});

class PaymentModeContainer extends React.Component {
    ...
    */
    getGridConfig(){
                ...
                },
                {
                    ...
                    Cell: row => (
                        <div>
                            ...
                            <a href="javascript:void(0)" className="red btn table-btn"  onClick={this.deleteData.bind(null,row.original.payment_mode_id)}>Delete</a>
                        </div>
                    )
                }
            ];
        ...
        const gridData = {
            noDataText : "Payment Mode not found !!",
            columns : gridCol,
            data                    : this.props.paymentModeData.result,
            minRows                 : 0,
            defaultPageSize         : 5,
            className               : "table table-bordered responsive",
            defaultFilterMethod     : (filter, row) =>String(row[filter.id]) === filter.value,
            filtered                : this.state.filtered,
            pages                   : this.props.paymentModeData.pages,
            showPagination          : true,
    ...
    */
    ...
    editHideHandle() {
        ...
        dispatch(paymentModeActions.resetState());
    ...
    showHandle(formDetail) { 
        ...
        const {formShow}  = this.state;
            ...
            if(formDetail != ''){
                ...
                fromTitle = 'Add New Payment Mode';
                for(var fc in fields){
                    var fieldName = fields[fc]['name'];
                    if(fields[fc]['type'] ==='tags'){
                        fields[fc]['value'] = [];
    ...
    */
    getTableList(page, pageSize, sorted, filtered){
        const { dispatch} = this.props;
        dispatch(paymentModeActions.getTablelist(page, pageSize, sorted, filtered));
    }
    ...
    /**
     * @DateOfCreation        04 Oct 2018
     * @ShortDescription      This function is responsible delete payment mode confirm alert
     * @param                 Event Object
     * @return                Nothing
     */
    deleteData(payment_mode_id){
        let formDetail = {};
        formDetail['payment_mode_id'] = payment_mode_id;
        confirmAlert({
          title: 'Payment Mode',
          message: <div className="alert-message">Are you sure you want to delete this payment mode?</div>,
          buttons: [
          {
            label: 'Yes',
            onClick: () => {
              const { dispatch } = this.props;
              dispatch(paymentModeActions.getDeleteRequest(formDetail));
            }
          },
          {
            label: 'No',
    ...
    handleSubmit() {
        if(this.boundForm){
            ...
            if(submitData){
                dispatch(paymentModeActions.submitRequest(submitData));
            }
       }
    }
    ...
    */
    componentWillReceiveProps(props) {
        ...
        if(props.dataGridRefresh){
            dispatch(paymentModeActions.getTablelist(0, 5, '', ''));
        }
...
function mapStateToProps(state) {
    ...
    const { isUserNotValid,isInsertDone, submitted, successMsg, errorMsg, paymentModeData,dataGridRefresh, pages, optionData, fetchOptionData } = state.paymentMode;
    return {
    	staticData,
    	staticDatafetched,
        isInsertDone,
...
const connectedPaymentModeContainer = connect(mapStateToProps)(PaymentModeContainer);
export { connectedPaymentModeContainer as PaymentModeContainer };
