 ...
 * @ShortDescription       This is responsible for calling all api related to add Checkup Type
 */
...
*/
export const checkupTypeService = {
    getTablelist,
    doSubmitRequest,
    doDeleteRequest
};
...
function doSubmitRequest(details) { 
    return axios({
            method: 'post',
            url: configConstants.API_BASE_PATH+'checkup-type/save',
...
function getTablelist(page, pageSize, sorted, filtered) {
    ...
    return axios({
            method  : 'post',
            url     : configConstants.API_BASE_PATH+'checkup-type/list',
            data    : dataForm,
            headers : headerHelper.getHeaderWithAuthorization()
        }).then(function (response) {
            return response;
...
function doDeleteRequest(details) {
    return axios({
        method  : 'delete',
        url     : configConstants.API_BASE_PATH + 'checkup-type/delete',
        headers : headerHelper.getHeaderWithAuthorization(),
        data    : headerHelper.appendUserDataInJson(details)
    })
    .then(response => {
